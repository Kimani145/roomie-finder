  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[9999] flex items-center justify-center bg-surface-dark-bg/95 backdrop-blur-md"
      >
        {/* Dismiss Button */}
        {phase === 'resolved' && (
        ...
        <div className="relative flex flex-col items-center">
          ...
          <div className="relative w-64 h-32 flex items-center justify-center mb-8">
             ...
             <motion.div
               ...
               className="absolute w-32 h-32 rounded-full bg-brand-600 shadow-2xl z-10 overflow-hidden border-4 border-surface-dark-bg"
             >
               {phase === 'resolved' && <img src="/favicon.svg" alt="You" className="w-full h-full object-cover opacity-50 bg-brand-50" />}
             </motion.div>

             {/* Right Circle (Success Green) */}
             <motion.div
               initial={{ x: 100, scale: 0.9, opacity: 0 }}
               animate={{
                 x: phase === 'animating' ? -20 : (phase === 'resolved' ? -20 : 100),
               ...
               transition={{ duration: ANIM_DURATION, ease: "easeOut" }}
               className="absolute w-32 h-32 rounded-full bg-accent-500 shadow-2xl mix-blend-multiply z-20 overflow-hidden border-4 border-surface-dark-bg"
             >
               {phase === 'resolved' && <img src={matchData.matchedUser.photoURL || '/favicon.svg'} alt="Match" className="w-full h-full object-cover bg-accent-50" />}
             </motion.div>
          </div>
          ...
          <AnimatePresence>
            {phase === 'resolved' && (
              ...
              >
                <div>
                  <h2 className="text-4xl font-syne font-bold text-accent-dark mb-2">It's a Match!</h2>
                  <p className="text-brand-300">You and {matchData.matchedUser.displayName} liked each other.</p>
                </div>

                <div className="flex flex-col w-full max-w-xs gap-3">
                  <button onClick={handleChat} className="w-full py-4 bg-brand-600 hover:bg-brand-500 text-white font-bold rounded-xl transition-all active:scale-[0.98]">
                    Start Chatting
                  </button>
